...
이제 보호 모드 커널의 가장 앞부분에 위치하는 코드인 `EntryPoint.s` 파일을 작성할 것이며 이것은 보호 모드 전환과 초기화를 수행하여 이후에 위치하는 코드를 위한 환경을 제공할 것이다.
...
Kernel32.bin: Source/EntryPoint.s
	nasm -o Temp/Kernel32.bin $<

clean:
	rm -f Temp/Kernel32.bin
```
`$<` : 이 매크로는 Dependency(:의 오른쪽)의 첫 번째 파일을 의미하는 매크로 이다. 따라서 `Source/EntryPoint.s`로 치환되며 이 엔트리 파일은 빌드되어 Kernel32.bin 파일로 생성된다.
...
`$^` : Dependency(:의 오른쪽)에 나열된 전체 파일을 의미한다. 따라서 `00.BootLoader/BootLoader.bin 01.Kernel32/Temp/Kernel32.bin`로 치환된다.

여기까지 작성하고 실행하면 성공했다는 메시지를 볼 수 없다. 이유는 부트 로더에 OS 이미지의 크기가 1024로 설정되어 있기 때문이다. 
빌드한 보호 모드 커널 이미지의 크기는 512바이트(1 섹터)밖에 되지 않으므로 부트 로더가 한 섹터를 로딩한 후 나머지 1023셑러를 읽으려다 정지한 것이다.
`BootLoader.asm`의 `TOTALSECTORCOUNT` 값을 변경해 주자

```
TOTALSECTORCOUNT: dw 1
```

이제 명령어를 실행하면 아래와 같은 화면을 볼 수 있다.
```
qemu-system-x86_64 -L . -m 64 -fda ./Disk.img -localtime -display curses
```

![switch protect mode](/contents/dev/2020/04/07/image/os-study-12-1.png)