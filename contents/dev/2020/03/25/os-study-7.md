...
이전에 <a href="https://knero.github.io/#/contents?path=/contents/dev/2020/03/20/os-study-5.md" target="_blank">부트로더 만들기</a> 에서 OS 이미지를 하드 디스크를 사용해서 로딩하도록  `-hda` 로 변경해서 사용해 왔지만 이제는 다시 플로피 디스크에 이미지를 로딩하기 위해 책에서와 같이 `-fda`로 변경했다.
